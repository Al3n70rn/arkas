...
they make sense.  How many transcripts make up these bundles, anyways?

```R 
hugeDiffs <- voomOnly_fdr001[1:9]
feats <- features(NS)[which(mcols(NS)$gene_id %in% hugeDiffs)]
featsByGene <- split(feats, feats$gene_id)
sapply(featsByGene, length)
```

So there's one bundle (a mitochondrial gene) that has one transcript, and the 
rest have between 5 and 14 transcripts apiece. This is where it helps to pull 
out Sleuth for inspection of the transcripts involved.

```R

# cf. ?withSleuth
covs <- as(colData(NS), "data.frame")
