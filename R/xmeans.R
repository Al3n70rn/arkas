#' X-means clustering: perform non-hierarchal clustering on a data matrix.
#'
#' References:
...
#' 
#' @param	x		 					a numeric matrix, or something coercible to a matrix
#' @param ik	 					the initial number of clusters applied to kmeans()
#' @param	maxiter 			maximum number of iterations allowed before convergence
#' @param	verbose				logical: if 'TRUE' the system outputs processing status
#' @param ignoreCov 		logical: if 'TRUE', covariances of cluster data ignored 
#' @param	mergeClusters logical: If 'TRUE', can merge clusters after division
#'
xmeans <- function(x, ik=2, maxiter=10, verbose=F, ignoreCov=T, mergeClusters=T){
	if (ik < 2) 
		ik <- 2
	x <- as.matrix(x)
	...
	for (i in 1:ik){ # for each cluster
		...
		lnL0 <- lnL(yi, yi.centers, ignoreCov)
		yi.lnL <- lnL0$lnL
		yi.detVx <- lnL0$detVx
		...
		# y$subcluster exist...
		if (verbose)	cat (paste("k1 =", k1, ", k2 =", k2,"\n"))
		if (nrow(yi) == 1){ # sample size is 1
			break
		}
		y <- split2cls(yi, yi.centers, q, bic.prior, lnL.prior, detVx.prior, maxiter, ignoreCov)
		...
		if (verbose) print(y$subcluster)
		if (verbose){ print(y$bic.prior)
				print(y$bic.post)
		...
		if (!y$continue){	# no-node
			...
			if (verbose)	cat ("no node and no stack...\n")
			break;
		}
		# splitting continues...
		y1 <- y$clj1	# data
	...
	#
	if (verbose) cat("merging after progressive dividing ...\n")

	k <- length(xcl$size)	# final cluster number
	if (k <= 2){	# minimum cluster number should be 2
		if (verbose) cat("merging skipped ...\n")
		return(list(cluster = xcl$cluster, centers = xcl$centers, size = xcl$size))
	}
	if (verbose){
		cat("xcl$detVx=")
	...
	if (verbose) print(klist)
	for (i in 1:(k-1)){ 
	    for (j in (i+1):k){ 
		k1 = klist[i]
		k2 = klist[j]
		if (verbose) cat(paste("inspecting the clusters", k1,"and", k2,"\n"))

		z <- mergedBIC(x, xcl, k1, k2, q, ignoreCov, verbose)
		if (z$ret == F){
			# k1 or k2 has been merged.
			# skip this roop
			if (verbose) cat("skipping... k1=", k1, "k2=", k2,"\n")
			next
		}
		if (z$bicdiv > z$bicmgd){
			# we prefer merged model.
			# replace larger cls. number to smaller cls. number
			if (verbose) cat("replace cls.", k2, "to", k1,"\n")
...
split2cls <- function(x, centers, q, bic.prior, lnL.prior, detVx.prior, maxiter, ignoreCov){
	if (is.null(bic.prior)){
		pb <- priorBIC(x, centers, q, ignoreCov)
		bic.prior <- pb$bic
		lnL.prior <- pb$lnL
		detVx.prior <- pb$detVx
	}
	bic.post <- postBICs(x, centers, q, maxiter, ignoreCov)
...
priorBIC <- function(x, centers, q, ignoreCov){
	lnL0 <- lnL(x, centers, ignoreCov)
...
postBICs <- function(x, centers, q, maxiter, ignoreCov){
	#
	# split to 2 clusters
	clsub <- kmeans(x, 2, maxiter)
...
mergedBIC <- function(x, xcl, k1, k2, q, ignoreCov, verbose){
	# sample size
	...
	if (n1 == 0 || n2 == 0){
		...
		if(verbose) cat(paste("already had been merged\n"))
		ret <- F
        	return( list (ret = ret))
	}
	...
	if (verbose) cat(paste("beta=", round (beta, digit=2), "\n"))
	...
	lnLmgd <- lnL(xmgd, ctrmgd, ignoreCov)
...
# ignoreCov: ignore the covariance 
lnL <- function(x, centers, ignoreCov=T){
	x <- as.matrix(x)
	p <- ncol(x)	# p-dimensional multivariate
