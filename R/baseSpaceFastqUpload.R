...
#' @param fastqFiles a vector of sample files to upload into basespace
...
baseSpaceFastqUpload<-function(headerFormat=c("SRA","Normal"), fastqPath,fastqFiles, sraOutputDir, basespaceProject  ) {

hFormatted <- match.arg(headerFormat, c("SRA", "Normal"))

#task: need to find arkas/inst/bin system.file() and invoke  the conversion pipe.
# requirements: the fastq file must be gzipped
stopifnot(lapply(fastqFiles,function(x)file.exists(paste0(fastqPath,"/",x)))=="TRUE")

stopifnot(lapply(fastqFiles,function(x) is.gzip(paste0(fastqPath,"/",x)))=="TRUE")
...
setwd(fastqPath)
command<-paste0(fastqHeaderConvert," ",fastqFiles," | ",fastqConvert2," | ", fastqConvert3," | gzip -c > ",sraOutputDir,"/",fastqFiles)
system(command)

#now to change the name to illumina std and upload to basespace
command2<-paste0(convertFastqToIlluminaStandard," ",sraOutputDir,fastqFiles," ",sraOutputDir)
system(command2) 
